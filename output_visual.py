...
# Create canvas setup
master = tk.Tk()
canvas_width, canvas_height = CANVAS_SIZE_1080P
ctx = tk.Canvas(master,
           width=canvas_width,
           height=canvas_height)
ctx.pack()

# Load and render game frame
frame = np.load("./example_down.npy")
pixel_dims = frame.shape
frame = cv2.resize(frame, (frame.shape[0] * IMAGE_UPSCALE, frame.shape[1] * IMAGE_UPSCALE))
img = ImageTk.PhotoImage(image=Image.fromarray(frame))
# Save rendering id of image to later inspect attributes
img_id = ctx.create_image(IMAGE_X, canvas_height / 2, anchor="c", image=img)
...
neuron_size = (canvas_height - (len(hidden_biases) * MIN_PADDING)) / len(hidden_biases)

# Render neuron nodes, saving calculated positions for weight rendering
hidden_pos = render_layer(ctx, render_rescale(hidden_biases, magnitude=1), 0, canvas_height, HIDDEN_LAYER_X, neuron_size)
out_pos = render_layer(ctx, render_rescale(output_biases, magnitude=1), 0, canvas_height, OUTPUT_LAYER_X, neuron_size, labels=OUTPUT_LABELS)
